...
function resolveCanvasFontFamily(family?: string) {
    ...
    if (typeof document === "undefined") return family || fallback

    const playfair = getComputedStyle(document.documentElement)
        .getPropertyValue("--font-playfair")
        .trim()

    if (!family || family.includes("var(--font-playfair)")) {
        return playfair || fallback
    }
...
export default function ParticleText(props: Props) {
    ...
    useEffect(() => {
        ...
        const sampleText = () => {
            ...
            const resolvedFamily = resolveCanvasFontFamily(fontFamily)
            ...
            offCtx.font = `${resolvedWeight} ${effectiveSize}px ${resolvedFamily}`
            ...
            // Hairlines (Playfair "e" bar, serifs) vanish between sample
            // points unless we fatten the glyph slightly before sampling.
            offCtx.lineWidth = Math.max(1.6, effectiveSize * 0.042)
            offCtx.fillText(text || "", W / 2, H / 2)
            offCtx.strokeText(text || "", W / 2, H / 2)
            ...
            // Particle Count (1–50): higher = denser. Tighter than the
            // original 150/count gap so thin glyph features still register.
            const pCount = Math.max(1, Math.min(50, particleCount))
            const stride = Math.max(1, Math.round(50 / pCount))

            const cellHasInk = (x: number, y: number) => {
                const x0 = Math.max(0, Math.floor(x * dpr))
                const y0 = Math.max(0, Math.floor(y * dpr))
                const x1 = Math.min(imgW, Math.floor((x + stride) * dpr))
                const y1 = Math.min(imgH, Math.floor((y + stride) * dpr))
                for (let iy = y0; iy < y1; iy++) {
                    for (let ix = x0; ix < x1; ix++) {
                        if (data[(iy * imgW + ix) * 4 + 3] > 72) return true
                    }
                }
                return false
            }
            ...
            cIdx = newC
            // Keep the word formed if it was already on screen; only replay
            // from spawn on a true first layout.
            if (hiddenRef.current || formValRef.current < 0.2) {
        ...
        const fitFontSize = (
            ...
            for (let iter = 0; iter < 12; iter++) {
                const mid = (lo + hi) / 2
                measureCtx.font = `${weight} ${mid}px ${family}`
